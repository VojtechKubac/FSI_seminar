...
#            return precice_data.ravel()
    ...
    def write_block_data(self):
        ...
        if  self._function_type is FunctionType.SCALAR: #write_field.value_rank() == 0:
            ...
            if self._fenics_dimensions == 2 and self._dimensions in [2, 3]:
                #This corresponds to 2D-3D coupling
                #zeros have to be inserted for the y-entry

                # 2D-2D coupling also enabled
    ...
    def create_coupling_neumann_boundary_condition(self, test_functions, boundary_marker=None, 
        ...
        """
        print("inside Neumann")
        #self._function_space = test_functions.function_space()
        # for MixedSpace does not work

        if function_space is not None:
            self._function_space = function_space
        else:
            try:
                self._function_sace = test_functions.function_space()
            except(AttributeError):     # MixedFunction case
                raise Exception("For this type of TestFunction function_spce needs to be specified.")


        print("coupling BC")
        self.create_coupling_boundary_condition()
        if not boundary_marker: # there is only 1 Neumann-BC which is at the coupling boundary 
                                # -> integration over whole boundary
            return dot(self._coupling_bc_expression, test_functions) * dolfin.ds  
                    # this term has to be added to weak form to add a Neumann BC 
                    # (see e.g. p. 83ff Langtangen, Hans Petter, and Anders Logg. 
                    # "Solving PDEs in Python The FEniCS Tutorial Volume I." (2016).)

        else: #For multiple Neumann BCs integration should only be performed over the respective domain.
              # TODO: fix the problem here
            print("create coupling form")
            ...
            return (self._coupling_bc_expression[0] * test_functions[0] \
                    + self._coupling_bc_expression[1] * test_functions[1]) * self.dss(boundary_marker)
    ...
    def initialize(self, coupling_subdomain, mesh, read_field, write_field, 
        ...
        #print('dim =', self._dimensions)
        #print('fenics_dim =', self._fenics_dimensions)
    ...
    def extract_coupling_boundary_coordinates(self):
        ...
        if self._dimensions == 2:
            ...
            raise Exception("Error: These Dimensions ({}, {}) are not supported by the adapter.".format(self._dimensions, self._fenics_dimensions))
